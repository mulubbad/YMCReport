...
const db = require('../db');
const { auth, requireRole, scopeGid, canManage } = require('../auth');
...
r.get('/activity/summary', adminOnly, (req, res) => {
  ...
  else if (req.user.role === 'admin') where.push('0');   // group-less admin leads nobody
...
r.get('/activity/:id', adminOnly, (req, res) => {
  ...
  if (req.user.role === 'admin' && !canManage(req.user, target.group_id))
