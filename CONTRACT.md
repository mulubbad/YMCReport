...
## Update cadence (دورية التحديث) — five metrics, the wizard, the compliance alert
**The required cadence is a property of the ACCOUNT TYPE**, set by a super or a team leader in Settings → أنواع الحسابات. `account_types.update_days INTEGER NOT NULL DEFAULT 1`: `0` = never (accounts of this type are never due and are excluded from every compliance count), `N` = must be updated at least once every N days. Existing types migrate to `1`, which is the original daily rule. Presets: 1 يومياً · 2 كل يومين · 3 كل 3 أيام · 7 أسبوعياً · 14 كل أسبوعين · 30 شهرياً · 0 بدون تحديث دوري — the map is `UPDATE_DAYS_AR` (routes/meta.js) on the server and `CADENCE` / `cadenceLabel` (`lib/cadence.ts`) on the client, which also renders `كل N يوم` for a custom value. The server accepts any integer 0–365; `meta.js`'s shared types/sites CRUD loop gained an optional per-entry `check(v)` hook, run after `values()` on **both** POST and PUT, that 400s anything else. PUT merges `{ ...row, ...req.body }` first, so a partial PUT (the Settings pages-toggle) preserves the stored cadence.

The ACCOUNT is the unit of compliance — a page is updated alongside its account but never counts. The *report* stays per-day regardless of cadence; only the *obligation* is windowed.

**The five metrics**, in this order everywhere (wizard steps, report columns, dashboard tiles). `followers` is the only **absolute** — you type what the platform shows and the system derives the gain; the other four are typed as **what is new since the last check**:

| key | Arabic label | entry |
|---|---|---|
| `followers` | المتابعون / الأصدقاء | absolute total, delta derived |
| `posts_count` | منشورات جديدة | delta |
| `friend_requests` | طلبات صداقة جديدة | delta |
| `groups_joined` | مجموعات انضم إليها | delta |
| `group_shares` | مشاركات في المجموعات | delta |

`shares` / `reactions` / `comments` stay in `TRACK` (the Facebook sync writes them) but are **secondary**: not wizard steps, not compliance metrics.

Schema: `accounts` + `pages` gain `friend_requests INTEGER`, `groups_joined INTEGER`, `group_shares INTEGER` (same pragma/ALTER migration loop as the other tracking columns). New table `account_daily (id, account_id → accounts ON DELETE CASCADE, page_id → pages ON DELETE CASCADE (NULL = the account itself), user_id → users ON DELETE SET NULL (the owner at the time, so a transferred account credits whoever did the work), day TEXT (local YYYY-MM-DD from notify.day()), followers INTEGER (latest absolute snapshot that day), d_followers, d_posts, d_friend_requests, d_groups_joined, d_group_shares, d_shares, d_reactions, d_comments INTEGER NOT NULL DEFAULT 0, checks INTEGER NOT NULL DEFAULT 0, updated_at)`, `UNIQUE(account_id, COALESCE(page_id,0), day)` + index `(day, user_id)`.

**`add_<metric>` on a quick update.** The four counters are sent as `add_posts_count`, `add_friend_requests`, `add_groups_joined`, `add_group_shares` — "this many are new" — and `quickUpdate()` resolves each against the row **as it is now** (`b[f] = (row[f] ?? 0) + inc`), rejecting a negative or non-integer with the field's Arabic error. An explicit absolute value still wins when both are sent, which is how `followers` keeps working. The client must NOT compute the total itself: a chained wizard run snapshots its targets when it starts, so a Facebook sync landing mid-run would make it post a stale, lower total — walking the counter backwards and recording a negative day.

`rollDaily()` in `routes/accounts.js` runs as the last statement of **`quickUpdate()`**, so it covers the wizard *and* the Facebook sync (which routes through it) — there is no second daily-write path. It runs **even when nothing changed**: a no-change «فحص» still marks the day done, which is what makes `account_daily` the single source for "was this updated today". Deltas **accumulate**, so three updates in one day sum correctly. The baseline rule applies to **`followers` only** — it is the one absolute, so a first-ever reading is a starting point, delta `0`. The other columns are counters whose posted value already IS the gain, so a null stored total counts as `0`, not as a baseline; treating them alike silently zeroes day one of every counter, and the three new columns are null on every pre-existing row. `followers` movement is signed and never clamped. `PUT /accounts/:id` does **not** roll daily: a profile edit is not a check.

**Due predicate** — `DUE_SQL` and the population it is measured against, `TRACKED_SQL`, both exported from `routes/accounts.js`, imported by `routes/tasks.js`, and mirrored verbatim in `notify.js` (which `accounts.js` requires, so it cannot import back):
```sql
-- DUE_SQL · binds exactly one arg, the local day from notify.day()
a.status = 'active' AND t.update_days > 0 AND NOT EXISTS (
  SELECT 1 FROM account_daily d WHERE d.account_id = a.id AND d.page_id IS NULL
    AND julianday(d.day) > julianday(?) - t.update_days)
-- TRACKED_SQL · the compliance population; a type that opted out is not "non-compliant", it is not counted
a.status = 'active' AND t.update_days > 0
```
**Every query using either MUST `JOIN account_types t ON t.id = a.type_id`.** A missing join is a silent wrong answer in SQLite, not an error — this is the single most likely way to break cadence. `julianday` over the day text, not a string-built date modifier, because the window is per-row: `N=1` means "a row today", `N=7` means "a row in the last 7 days including today". The boundary is `>=`: a last row exactly N days old IS due. The client rule in `Accounts.tsx` (`isDue` / `dayGap`, UTC-anchored `Date.parse` on the `YYYY-MM-DD` text so DST cannot shift it) must return the same verdict as the server for every account, or a row badge contradicts the banner above it.

It keys off `account_daily.day` (the server's **local** day) rather than `last_checked_at` (stored UTC) — comparing the two would mark every account overdue for the first hours of each day in UTC+2/+3.

API:
- `ACCOUNT_SQL` rows gain `last_daily_day` (the most recent account-level `account_daily.day`, null if never) and `update_days` (from the joined type); the UI needs both for `isDue`.
- **`GET /accounts/daily`** — the wizard queue, and the **only** source of `yesterday_followers`. Same scope rules as `GET /accounts`. → `{ day, due: [<account row> + yesterday_followers], done, total }`, `yesterday_followers` being the `followers` snapshot from that account's most recent daily row **before** today; `total` is the `TRACKED_SQL` population, so an opted-out type never inflates `done/total`. The chained run must load its targets from here, not filter the loaded list: the list rows carry no `yesterday_followers`, and this route is also the authority on what is still due.
- `GET /stats` gains `daily_due` (`DUE_SQL`) / `daily_total` (`TRACKED_SQL`) for **every** role (0 in the group-less-admin early return), and `detail.daily = { date, accounts, updated, due, rate, totals:{followers,posts,friend_requests,groups_joined,group_shares}, series:[{date,updated,…the five}] (zero-filled per day in range), members:[{id,name,group_name,accounts,updated,due,rate,…the five,last_update_at}] }`. `daily.members` is **every member in scope plus every other account owner** — a leader who adds an account for themselves owns it (POST /accounts defaults the owner to the caller), and those accounts are in `accounts`/`updated`, so their owner has to be a row or the headline can never equal the sum of the table. It is not sliced to 20 like `detail.members`. `followers` totals are `SUM(d_followers)` (net, may be negative).

Notifications — two kinds, both targeted (neither is in `UPDATE_KINDS`, so super is **not** copied):
- `account_daily_due` → the **owner**, one per user per day, key `daily:{userId}:{YYYY-MM-DD}`, title "{n} حساب بانتظار التحديث", link `/accounts?daily=1`. Generated only after **`OWNER_NUDGE_HOUR = 12`**.
- `group_daily_due` → each **leader** of the group, one per group per day, key `daily:group:{gid}:{YYYY-MM-DD}`, title "{n} حساب تجاوز موعد التحديث", body the worst three members as "{name} ({count})", link `/`. Generated only after **`DAILY_DIGEST_HOUR = 16`**, and gated on `role !== 'user'` over `ledIds()` — a super who also leads the team is a leader here.
Both hour gates exist because the key is per-day and `INSERT OR IGNORE` means a count written at 00:05 can never be corrected or withdrawn: at midnight every account is due by construction, so a member who finishes all twelve by 09:30 would otherwise keep a red "12 awaiting" in the bell all day. The always-on banner on /accounts is the morning prompt; these notifications are the late reminder. Icons/labels (one shared `KINDS` map in `components/Notifications.tsx`, which the archive page imports): `account_daily_due` = CalendarClock danger «تحديث دوري مطلوب», `group_daily_due` = CalendarX2 danger «حسابات تجاوزت موعد التحديث».

Export: `TRACK_COLS` / `track()` gain طلبات الصداقة، المجموعات المنضم إليها، المشاركات في المجموعات (positional and shared by the accounts *and* pages sheets — both arrays change together or every later cell in both shifts silently). The `accounts` sheet alone gains **دورية التحديث** immediately after النوع, from `UPDATE_DAYS_AR` with a `كل N يوم` fallback; it lives in that sheet's own column list, deliberately **outside** `TRACK_COLS`.
New sheet key `daily` "التقدم اليومي" between `interactions` and `summary` — **activity, not compliance**: once types carry different windows a per-day ratio is meaningless (a weekly account untouched today is not behind), so it has no «متبقية»/«نسبة الالتزام», and no tracked-accounts column either — that one counted a different population from the updated count beside it, so a member with only opted-out types could read «0 tracked, 3 updated». Columns: التاريخ، المستخدم، حسابات حُدِّثت، متابعون جدد، منشورات جديدة، طلبات صداقة، مجموعات جديدة، مشاركات في المجموعات, newest day first, honouring `gid/user_ids/from/to`. Rows stay unfiltered by cadence: growth on an opted-out account is still growth.
Compliance lives on `summary` as **الالتزام %**: expected = `Σ over the member's TRACKED accounts of ceil(spanDays / that type's update_days)` (one bucket per cadence, not a single multiply), actual = that member's `account_daily` row count **restricted to the same tracked population** — counting the numerator over every account while the denominator excludes opted-out types inflates the rate. `to` is clamped to today, so a whole-month export run on the 21st does not count the nine future days as missed.

UI:
- **`components/DailyUpdate.tsx`** replaces the old «تحديث سريع» dialog entirely (`emptyQuick`/`quickForm`/`saveQuick`/`DELTAS` are gone). Props `{ open, targets: DailyTarget[], kind?: 'account'|'page', onOpenChange, onSaved }` — `targets.length === 1` is the single-account edit the ⚡ buttons still open, `> 1` is the chained run. Its copy is cadence-neutral («التحديث الدوري — {name}», «اكتمل التحديث», «لا {nounPl} بانتظار التحديث»), since “اليوم” is false for any non-daily type. Three steps per target: **النمو** (followers absolute + friend_requests) → **النشاط** (posts_count, group_shares, groups_joined) → **المراجعة** (status + note + a read-only summary of exactly what will be saved). The rail shows «الخطوة 2 من 3» and, in a chain, «الحساب 3 من 7» with a determinate bar and a per-target check/dash. Large centred `inputMode="numeric"` hero input; `+1 / +5 / +10` chips under each delta; a live «آخر قراءة {n} · جديد اليوم ▲ +{d}» readout (labelled *آخر قراءة*, not *أمس*: the previous snapshot may be days old if a day was missed, and on the single ⚡ path it falls back to the stored total); «لا جديد» zero-fills a step and advances; Enter advances; drafts autosave to `localStorage['ymc:daily:{kind}:{id}:{day}']` (try/catch, cleared on save) and closing with unsaved values confirms first; an end-of-run summary replaces a toast per account. An untouched account still POSTs `{}` via «سجّل فحصًا بلا تغيير» so the day is marked done; «تخطّي» posts nothing and leaves it due. RTL: forward is `ChevronLeft`. Two non-obvious bindings: `onKeyDown` sits on `DialogContent`, not on the step body — that body is keyed per step, so moving to المراجعة (no number box) unmounts the focused element and Radix returns focus to the container, above any handler bound deeper. And the last real target is held in a ref, because `targets` empties in the same commit that closes the dialog while Radix keeps the content mounted for its 200ms exit — rendering the empty state there would flash a false «لا حسابات بانتظار التحديث» on the way out.
- **Accounts page**: a `role="alert"` danger banner above the list when anything is due — count, one line of guidance, one action «ابدأ التحديث» which loads `GET /accounts/daily`; hidden entirely when nothing is due. A «بانتظار التحديث» quick-filter chip with its count, and the `?daily=1` deep link the notification points at. The per-row badge is three-way: due → danger «متأخر عن التحديث»; tracked and inside its window → success «مُحدَّث»; `update_days = 0` → **no badge at all**, since the type opted out and a badge would be noise. Each carries `title={cadenceLabel(update_days)}` so the window is discoverable. The الإحصائيات tab shows the three new counters and the type's cadence.
- **Settings → أنواع الحسابات** is where the cadence is set (already admin/super-only, so no new permission): a `Select` fed by `CADENCE` with helper text, plus a badge in the types list — `secondary` normally, `warning` at `0`, because an opted-out type is a deliberate exception and should read as one rather than as an error. Any partial PUT from this tab must carry `update_days`.
- **Dashboard**: one «التقدم اليومي» card under the date-range strip (the card keeps its name — the *report* is per-day whatever the cadence) — compliance as `{updated}/{accounts} ضمن دوريتها` + rate on a determinate bar, with a danger line `{due} حساب بانتظار التحديث` to `/accounts?daily=1`. The zero-due line reads «كل الحسابات المتتبَّعة ضمن دوريتها» (never “حُدِّثت اليوم”, which is false for any non-daily type) and the zero-tracked line names what is actually zero, «لا توجد حسابات ضمن دورية تحديث…», because `daily.accounts` is the TRACKED count and not the active one. Then the five range totals as one tabular row, the per-day strip from `daily.series`, and the per-member table. Admin/super only. `StripCfg.mainTotal` exists for this card: one bar is "accounts updated that day", but summing thirty of them counts *updates*, so the legend and a weekly-bucket tooltip say «تحديثاً» while a daily bar still says «حسابات محدّثة».
...
Table `notifications`: `id, user_id → users ON DELETE CASCADE, key TEXT NOT NULL, kind TEXT (task_new|task_due_soon|task_overdue|task_done|account_stale|account_status|account_daily_due|group_daily_due|task_nudge|message|mention|comment — CHECK enum; db.js rebuilds older tables once), title TEXT NOT NULL, body TEXT, link TEXT (SPA route), read INTEGER NOT NULL DEFAULT 0, created_at`; `UNIQUE(user_id, key)` (idempotent generation via INSERT OR IGNORE), index (user_id, id).
...
Push payload adds `unread` (stringified count) so the SW can sync the app-icon badge. `push()` truncates title (200) / body (300); a token is deleted only on 404 or a 400 whose body says UNREGISTERED/registration token (other 400s = malformed payload, tokens kept). `module.exports.enabled()` → GET /stats gains `push_enabled` so the UI can surface a server missing `FIREBASE_SERVICE_ACCOUNT`.
Frontend: `lib/push.ts` — Firebase web config (project `ymc-team`) + `getToken` against the PWA's own service worker (VAPID key env `VITE_FIREBASE_VAPID_KEY`; "unsupported" when missing). `forgetPush()` also unsubscribes the local PushManager subscription (authoritative even when the server DELETE fails). Bell banner covers: default (تفعيل), granted-but-unbound (retry), denied (hint to browser settings), iOS-not-installed (add-to-home-screen hint). `public/push-sw.js` is `importScripts`'d into the workbox SW: on `push` it flushes the offline mutation queues (registers the `workbox-background-sync:api-*` sync tags), syncs the app-icon badge (`navigator.setAppBadge(unread)`), posts `{type:'push'}` to open windows (→ `ymc:refresh` → poll + in-app toast; the archive page listens too) and shows an OS notification (RTL, app icon, `tag` = key) — suppressed when a window is visible on Chromium only (WebKit revokes subscriptions after ~3 silent pushes, so Safari/iOS always shows); `notificationclick` focuses a window and posts `{type:'navigate', link}` (or opens one). Bell panel shows a "فعّل تنبيهات الجهاز" row while permission is `default`; `syncPush()` re-registers the token on every app load when granted; `logout()` calls `forgetPush()` to unbind the device.
...
**A super may ALSO lead teams.** Leadership is a *membership* (`admin_groups`), not a permission level, so the two are
orthogonal: adding a super to a group puts them in the team — they appear in «نبض الفريق», `groupAdmins()` reaches them,
and they receive `group_daily_due` and the other leader notifications — while `managedIds()` still returns `null` for
them, so it narrows nothing. `ledIds(id)` (auth.js) is the role-agnostic membership read; `managedIds()` is the scope
question and is the only one that answers `null`. Anything that asks "is this person a leader of group G" must use
`admin_groups` membership, never `role = 'admin'` — `groupAdmins()`, `/tasks/team`'s `admins`, and the daily digest all
test `u.role != 'user'` over that join. `syncAdminGroups()` (core.js) writes the table for any `role !== 'user'` and
falls back to `ledIds`, never `managedIds`: the latter is `null` for a super and would wipe their leadership on any
user update that did not carry `group_ids`. The Users form offers the checkboxes as «الفرق التي يقودها» for a super.

**Server** — `server/src/auth.js` owns the whole rule; no route re-implements it:
- `managedIds(me)` → super `null` (= every group, led or not) · admin `admin_groups` rows · member `[group_id]`.
- `ledIds(id)` → the groups this user LEADS, whatever their role. Membership, not scope — see the super note above.
