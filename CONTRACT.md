...
## Daily update (التحديث اليومي) — five metrics, the wizard, the compliance alert
Every **active** account must be updated **once per local calendar day**. The day is the unit of the reminder and of the report; a page is updated alongside its account, but only the ACCOUNT is the unit of compliance.
...
**Due predicate** — one definition, exported as `DUE_TODAY_SQL` from `routes/accounts.js`, imported by `routes/tasks.js` and mirrored verbatim in `notify.js` (which `accounts.js` requires, so it cannot import back):
```sql
a.status = 'active' AND NOT EXISTS (
  SELECT 1 FROM account_daily d WHERE d.account_id = a.id AND d.page_id IS NULL AND d.day = ?)   -- bind notify.day()
```
...
API:
- `ACCOUNT_SQL` rows gain `last_daily_day` (the most recent account-level `account_daily.day`, null if never); the UI compares it to `today()` for the per-row badge.
- **`GET /accounts/daily`** — the wizard queue, and the **only** source of `yesterday_followers`. Same scope rules as `GET /accounts`. → `{ day, due: [<account row> + yesterday_followers], done, total }`, `yesterday_followers` being the `followers` snapshot from that account's most recent daily row **before** today. The chained run must load its targets from here, not filter the loaded list: the list rows carry no `yesterday_followers`, and this route is also the authority on what is still due.
- `GET /stats` gains `daily_due` / `daily_total` for **every** role (0 in the group-less-admin early return), and `detail.daily = { date, accounts, updated, due, rate, totals:{followers,posts,friend_requests,groups_joined,group_shares}, series:[{date,updated,…the five}] (zero-filled per day in range), members:[{id,name,group_name,accounts,updated,due,rate,…the five,last_update_at}] }`. `daily.members` is **every member in scope plus every other account owner** — a leader who adds an account for themselves owns it (POST /accounts defaults the owner to the caller), and those accounts are in `accounts`/`updated`, so their owner has to be a row or the headline can never equal the sum of the table. It is not sliced to 20 like `detail.members`. `followers` totals are `SUM(d_followers)` (net, may be negative).

Notifications — two kinds, both targeted (neither is in `UPDATE_KINDS`, so super is **not** copied):
- `account_daily_due` → the **owner**, one per user per day, key `daily:{userId}:{YYYY-MM-DD}`, title "{n} حساب بانتظار تحديث اليوم", link `/accounts?daily=1`. Generated only after **`OWNER_NUDGE_HOUR = 12`**.
- `group_daily_due` → each **leader** of the group, one per group per day, key `daily:group:{gid}:{YYYY-MM-DD}`, title "{n} حساب لم يُحدَّث اليوم", body the worst three members as "{name} ({count})", link `/`. Generated only after **`DAILY_DIGEST_HOUR = 16`**, and gated on `role !== 'user'` over `ledIds()` — a super who also leads the team is a leader here.
Both hour gates exist because the key is per-day and `INSERT OR IGNORE` means a count written at 00:05 can never be corrected or withdrawn: at midnight every account is due by construction, so a member who finishes all twelve by 09:30 would otherwise keep a red "12 awaiting" in the bell all day. The always-on banner on /accounts is the morning prompt; these notifications are the late reminder. Icons: `account_daily_due` = CalendarClock danger, `group_daily_due` = CalendarX2 danger.

Export: `TRACK_COLS` / `track()` gain طلبات الصداقة، المجموعات المنضم إليها، المشاركات في المجموعات (positional — both arrays change together or every later cell in the accounts *and* pages sheets shifts silently). New sheet key `daily` "التقدم اليومي" between `interactions` and `summary`: one row per **day × member** — التاريخ، المستخدم، الحسابات، محدّثة، متبقية، نسبة الالتزام %، متابعون جدد، منشورات جديدة، طلبات صداقة، مجموعات جديدة، مشاركات في المجموعات — honouring `gid/user_ids/from/to`, newest day first. The `summary` sheet gains الالتزام اليومي %، متابعون جدد، منشورات جديدة، طلبات صداقة، مشاركات في المجموعات at the end; its compliance denominator is `active accounts × days in range` with **`to` clamped to today**, so a whole-month export run on the 21st does not count the nine days that have not happened yet as missed.

UI:
- **`components/DailyUpdate.tsx`** replaces the old «تحديث سريع» dialog entirely (`emptyQuick`/`quickForm`/`saveQuick`/`DELTAS` are gone). Props `{ open, targets: DailyTarget[], kind?: 'account'|'page', onOpenChange, onSaved }` — `targets.length === 1` is the single-account edit the ⚡ buttons still open, `> 1` is the chained daily run. Three steps per target: **النمو** (followers absolute + friend_requests) → **النشاط** (posts_count, group_shares, groups_joined) → **المراجعة** (status + note + a read-only summary of exactly what will be saved). The rail shows «الخطوة 2 من 3» and, in a chain, «الحساب 3 من 7» with a determinate bar and a per-target check/dash. Large centred `inputMode="numeric"` hero input; `+1 / +5 / +10` chips under each delta; a live «آخر قراءة {n} · جديد اليوم ▲ +{d}» readout (labelled *آخر قراءة*, not *أمس*: the previous snapshot may be days old if a day was missed, and on the single ⚡ path it falls back to the stored total); «لا جديد» zero-fills a step and advances; Enter advances; drafts autosave to `localStorage['ymc:daily:{kind}:{id}:{day}']` (try/catch, cleared on save) and closing with unsaved values confirms first; an end-of-run summary replaces a toast per account. An untouched account still POSTs `{}` via «سجّل فحصًا بلا تغيير» so the day is marked done; «تخطّي» posts nothing and leaves it due. RTL: forward is `ChevronLeft`. Two non-obvious bindings: `onKeyDown` sits on `DialogContent`, not on the step body — that body is keyed per step, so moving to المراجعة (no number box) unmounts the focused element and Radix returns focus to the container, above any handler bound deeper. And the last real target is held in a ref, because `targets` empties in the same commit that closes the dialog while Radix keeps the content mounted for its 200ms exit — rendering the empty state there would flash a false «لا حسابات بانتظار تحديث اليوم» on the way out.
- **Accounts page**: a `role="alert"` danger banner above the list when anything is due — count, one line of guidance, one action «ابدأ التحديث اليومي» which loads `GET /accounts/daily`; hidden entirely when nothing is due. A «بانتظار تحديث اليوم» quick-filter chip with its count, a per-row «لم يُحدَّث اليوم» / «حُدِّث اليوم» badge, and the `?daily=1` deep link the notification points at. The الإحصائيات tab shows the three new counters.
- **Dashboard**: one «التقدم اليومي» card under the date-range strip — today's compliance (`{updated}/{accounts}` + rate on a determinate bar, with a danger line to `/accounts?daily=1`), the five range totals as one tabular row, the per-day strip from `daily.series`, and the per-member table. Admin/super only. `StripCfg.mainTotal` exists for this card: one bar is "accounts updated that day", but summing thirty of them counts *updates*, so the legend and a weekly-bucket tooltip say «تحديثاً» while a daily bar still says «حسابات محدّثة».
